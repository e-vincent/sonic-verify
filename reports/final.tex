...
\begin{lstlisting}
    ...
    k?(x$_1$, ..., x$_n$) in P
    k $\triangleleft$ $\ell$; P  
    k $\triangleright$ {$\ell$$_1$:P$_1$[] ... []$\ell$$_n$:P$_n$}
    throw k[k$_0$]; P       
    catch k(k$_0$) in P 
\end{lstlisting}
	\end{minipage}
...
\begin{myitemize}
  ...
  \item[] : Channel Sending
  \item[] : Channel Receiving
\end{myitemize}
	\end{minipage}
...
\begin{lstlisting}
    ...
    ($v$a)P ($v$k)P 
    if e then P else Q 
    def X$_1$($\tilde{x}$$_1$ $\tilde{k}$$_1$) = P$_1$ and 
       ... and X$_n$($\tilde{x}$$_n$ $\tilde{k}$$_n$) = P$_n$ in P
\end{lstlisting}
	\end{minipage}
...
% Rewrite the formatting for these bits
% Also write up a working example of binary ST
%    ATM examples from course would be a good one -> cite Nobuko?

\paragraph{Example Situation}
To put the information discussed here into some context, this next section
introduces a basic example of session types througha basic ATM system\footnote{
A simplified recreation of the example found in \cite{HVM98}, section 4}. 
In this situation there is one \texttt{ATM} agent that offers two services, 
balancing or depositing. These services are offered as two branch 
\emph{labels}. Firstly, the user will establish a \emph{session} via $a$. The 
user can then input their verification code (validation of this is ommitted
for the current example). If \emph{balance} is chosen, then the 
\texttt{ATM} requests the value from the bank and shows this to the user, before
returning to the menu state. If \emph{deposit} is selected, the 
\texttt{ATM} accepts the deposited amount to be sent to the bank, and again
returns to the menu state. A user may exit at any time via \emph{quit}. This 
effectively demonstrates continuous unbounded interactions in session types.

The following implementation demonstrates this agent:
\\
\begin{lstlisting}
  ATM($a$,$y$)       = accept $a$($z$) in $k$![$id$]. ATM$_1$[$a$,$y$,$id$,$z$]
  ATM$_1$($a$,$y$,$id$,$s$) = $s$ $\triangleright$ { $\emph{deposit}$: request $y$($h$) in $s$?($amt$) in
                         $h \triangleleft \emph{deposit}$. $h$![$id$, $amt$]. ATM$_1$[$a$,$y$,$id$,$s$]
                    [] $\emph{balance}$: request $y$($h$) in 
                         $h \triangleleft \emph{balance}$. $h$?($amt$) in
                              $s$![$amt$]. ATM$_1$[$a$,$y$,$id$,$s$]
                    [] $\emph{quit}$: ATM($a$, $y$) }
\end{lstlisting}

A simple user agent may look as follows:
\\
\begin{lstlisting}
  request $a$($z$) in $z$![$myID$]. $z \triangleleft \emph{quit}$ 
\end{lstlisting}
...
From this we can now define the communication type system for multi-party 
session types\cite{CCPY15}. In this, \emph{Global Types} describe the whole
conversation scenario. \emph{Session Types} are constructed through 
\emph{projection} of the global type on the individual participants of
the scenario. The grammar for this is presented next:
...
global type \texttt{G} (or \texttt{G$_i$} in the case of branching). In this 
grammar, the style of recursive types uses \emph{equi-recursion}; there is no
distinction between the type \texttt{${\mu}$t.G} and the unfolding 
\texttt{G\{${\mu}$t.G/t\}}.

The session types represent the actions performed by single participants. For 
example, the \emph{send} type, \texttt{!<p, S>.T} describes the sending of a 
value of sort S to participant \texttt{p}. Recursion is guarded in session types
as they are in global types.

The concept of \emph{projection} between global types and session types is
formalised in Definition 1, as found in \cite{HYC08}.
...
\end{align*}
\begin{align*}
	(\texttt{p} \rightarrow \texttt{p'}:\texttt{p',\{l$_{i}:G_{i}$\}$_{i \in I}$}) \upharpoonright \texttt{q = }  
	\begin{cases}
\hspace{5pt} \oplus \langle \texttt{p',\{l$_{i}:T_{i}$\}$_{i \in I}$} \rangle (\texttt{$G$'}\upharpoonright \texttt{q}) \hspace{22pt}if \hspace{5pt} \texttt{q = p} \\
\hspace{5pt} \&(\texttt{p',\{l$_{i}:G_{i}$}\upharpoonright \texttt{q\}$_{i \in I}$}) \hspace{40pt} if \hspace{5pt} \texttt{q = p'} \\
\hspace{5pt} \texttt{$G_{i_0}$}\upharpoonright \texttt{q} \hspace{77pt} where \hspace{5pt} \texttt{$i_{0}$} \in I \hspace{5pt} if \hspace{3pt} \texttt{q} \neq \texttt{p,}\hspace{3pt}\texttt{q} \neq \texttt{p'} \\
\hspace{115pt} and \hspace{3pt} \texttt{$G_{i}$} \upharpoonright \texttt{q} = \texttt{$G_{j}$} \upharpoonright \texttt{q} \hspace{3pt} for\hspace{3pt} all\hspace{3pt} i,\hspace{3pt} j \in I
	\end{cases}
\end{align*}
\begin{align*}
	(\mu\texttt{t$G$}) \upharpoonright \texttt{q = }
	\begin{cases}
\hspace{5pt} \mu\texttt{t}(\texttt{$G$} \upharpoonright \texttt{q}) \hspace{20pt} if \hspace{5pt} \texttt{$G$} \upharpoonright \texttt{q} \neq \texttt{t} \\
\hspace{5pt} \texttt{end} \hspace{48pt} otherwise
	\end{cases} 
\end{align*}
\begin{align*}
...
\paragraph{Example Situation}
To fully illustrate the basic featurs and functionalities of the calculus we
discuss a standard two-buyers-protocol, demonstrated here as a sequence 
diagram:
...
\begin{minipage}{\textwidth}
	...
	\end{lstlisting}
	\captionof{lstlisting}{Forming Types}
\end{minipage}

Initially when forming the types of threads, the program of snippet 11
...
\begin{minipage}{\textwidth}
	...
	\end{lstlisting}
	\captionof{lstlisting}{Deadlock - types under this approach}
\end{minipage}
...
To handle this, the project makes use of sub-typing the \texttt{cue} type\footnote{
As suggested by Dominic Orchard in his presentation on `Verifying and Timing
